    ...
    try {
        ...
        for (FIT_UINT16 i = 0; i < 3600; i++) {
            ...
            recordMesg.SetHeartRate(
                (FIT_UINT8) ((sin(TWOPI * (0.01 * i + 10)) + 1.0) * 127.0));        // Sine
        ...
        lapMesg.SetTotalElapsedTime(
            (FIT_FLOAT32) (timestamp.GetTimeStamp() - startTime.GetTimeStamp()));
        lapMesg.SetTotalTimerTime(
            (FIT_FLOAT32) (timestamp.GetTimeStamp() - startTime.GetTimeStamp()));
        ...
        activityMesg.SetLocalTimestamp(
            (FIT_LOCAL_DATE_TIME) ((int) timestamp.GetTimeStamp() + timezoneOffset));
...
int EncodeMonitoringFile() {
    try {
        ...
        file.open("ExampleMonitoringFile.fit",
                  std::ios::in | std::ios::out | std::ios::binary | std::ios::trunc);
        ...
        deviceInfoMesg.SetTimestamp(
            initTime.GetTimeStamp()); // Convert to FIT time and write timestamp.
        ...
        // By default, each time a new message is written the Local Message Type 0 will be
        // redefined to match the new message. In this case,to avoid having a definition message
        // each time there is a DeviceInfoMesg, we can manually set the Local Message Type of the
        // MonitoringMessage to '1'. By doing this we avoid an additional 7 definition messages in
        // our FIT file.
        ...
        for (int i = 0; i < 4; i++)                           // This loop represents 1/6 of a day
        {
            for (int j = 0; j < 4; j++) // Each one of these loops represent 1 hour
            {
                ...
                monitoringMesg.SetActivityType(
                    FIT_ACTIVITY_TYPE_WALKING); // By setting this to WALKING, the Cycles field
                                                // will be interpretted as Steps
                monitoringMesg.SetCycles(
