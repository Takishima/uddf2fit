    ...
    do {
        ...
        for (; currentByteIndex < bytesRead; currentByteIndex++) {
            ...
            switch (decodeReturn) {
                ...
                case RETURN_MESG:
                    if (mesg.GetNum() == FIT_MESG_NUM_DEVELOPER_DATA_ID) {
                        ...
                        if (!descMesg.IsFieldDefinitionNumberValid()) {
                            throw fit::RuntimeException("Invalid developer field definition "
                                                        "number in FieldDescriptionMesg");
                        }
                ...
                case RETURN_END_OF_FILE:
                    // Increment so we do not read the same byte twice in the case of a chained
                    // file
                    currentByteIndex++;
                    currentByteOffset++;
                    return FIT_TRUE;
...
Decode::RETURN Decode::ReadByte(FIT_UINT8 data) {
    if ((fileBytesLeft > 0) && (skipHeader == FIT_FALSE)) {
        ...
        if (fileBytesLeft == 1) // CRC low byte.
        {
            if (state != STATE_RECORD) {
                std::ostringstream message;
                message << "FIT decode error: Decoder not in correct state after last data byte "
                           "in file. Check message definitions. Error at byte: "
                        << currentByteOffset;
                throw(RuntimeException(message.str()));
            }
    ...
    switch (state) {
            ...
            switch (fileHdrOffset++) {
                case 0:
                    if (data < FIT_HEADER_SIZE_NO_CRC) {
                        std::ostringstream message;
                        message << "FIT decode error: File header size invalid.  File is not FIT. "
                                   "Error at byte : "
                                << currentByteOffset;
                ...
                case 8:
                    if (data != '.') {
                        std::ostringstream message;
                        message << "FIT decode error: File header signature mismatch.  File is "
            ...
            if (fileBytesLeft > 1) {
                if ((data & FIT_HDR_TIME_REC_BIT) != 0) {
                    ...
                    if (localMesgDefs[localMesgIndex].GetNum() == FIT_MESG_NUM_INVALID) {
                        std::ostringstream message;
                        message << "FIT decode error: Missing FIT message definition for local "
                                   "message number "
                                << ((int) localMesgIndex)
                    ...
                    if ((data & FIT_HDR_TYPE_DEF_BIT) != 0) {
                        ...
                        if (localMesgDefs[localMesgIndex].GetFields().size() != 0) {
                            state = STATE_FIELD_DATA;
                        } else if (localMesgDefs[localMesgIndex].GetDeveloperFieldTotalSize()
                                   > 0) {
        ...
        case STATE_MESG_NUM_1:
            localMesgDefs[localMesgIndex].SetNum(localMesgDefs[localMesgIndex].GetNum()
                                                 | ((FIT_UINT16) data << 8));
        ...
        case STATE_DEV_FIELD_INDEX:
            ...
            try {
                const FieldDescriptionMesg &desc = descriptions.at(fieldData[DevFieldIndexOffset])
                ...
                localMesgDefs[localMesgIndex].AddDevField(
                    DeveloperFieldDefinition(fieldData[DevFieldNumOffset],
        ...
        case STATE_FIELD_DATA:
            if (fieldBytesLeft == 0) {
                fieldDataIndex = 0;
                fieldBytesLeft
                    = localMesgDefs[localMesgIndex].GetFieldByIndex(fieldIndex)->GetSize();
                ...
                if (baseType < FIT_BASE_TYPES) // Ignore field if base type not supported.
                {
                    ...
                    if (field.IsValid()) // If known field type.
                    {
                        if (field.GetType() != fldDefn->GetType()) {
                            FIT_UINT8 profileSize
                                = fit::baseTypeSizes[(field.GetType() & FIT_BASE_TYPE_NUM_MASK)];
                        ...
                        // Allows messages containing the accumulated field to set the accumulated
                        // value
            ...
            if (fieldIndex >= localMesgDefs[localMesgIndex].GetFields().size()) {
                // Now that the entire message is decoded we may evaluate subfields and expand
                // components
                for (FIT_UINT16 i = 0; i < mesg.GetNumFields(); i++) {
                    ...
                    if (!suppressComponentExpansion) {
                        if (activeSubField == FIT_SUBFIELD_INDEX_MAIN_FIELD) {
                            ...
                            if (mesg.GetFieldByIndex(i)->GetSubField(activeSubField)->numComponents
                                > 0) {
                                ExpandComponents(mesg.GetFieldByIndex(i),
...
void Decode::SuppressComponentExpansion(void) { suppressComponentExpansion = FIT_TRUE; }
...
void Decode::ExpandComponents(Field *containingField,
    ...
    for (i = 0; i < numComponents; i++) {
        ...
        if (component->num != FIT_FIELD_NUM_INVALID) {
            ...
            if (componentField.IsSignedInteger()) {
                ...
                if (component->accumulate)
                    bitsValue = accumulator.Accumulate(
                        mesg.GetNum(), component->num, signedBitsValue, component->bits);
            } else {
                ...
                if (component->accumulate)
                    bitsValue = accumulator.Accumulate(
                        mesg.GetNum(), component->num, bitsValue, component->bits);
            }

            // If the component field itself has *one* component apply the scale and offset of the
            // componentField's (nested) component
            if (componentField.GetNumComponents() == 1) {
                if (componentField.IsSignedInteger())
                    value
                        = (((signedBitsValue / (FIT_FLOAT64) component->scale) - component->offset)
            ...
            // The component field is itself a composite field (more than one component).  Don't
            // use scale/offset, containing field data must already be encoded.  Add elements to it
            // until we have added bitsvalue
