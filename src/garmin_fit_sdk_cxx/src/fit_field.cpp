...
Field::Field(void)
    : FieldBase(), profile(NULL), type(FIT_UINT8_INVALID), isFieldExpanded(FIT_FALSE) {}
...
Field::Field(const Profile::MESG_INDEX mesgIndex, const FIT_UINT16 fieldIndex)
    : FieldBase(), profile(&Profile::mesgs[mesgIndex]), profileIndex(fieldIndex),
      type(FIT_UINT8_INVALID), isFieldExpanded(FIT_FALSE) {}

Field::Field(const FIT_UINT16 mesgNum, const FIT_UINT8 fieldNum)
    : FieldBase(), profile(Profile::GetMesg(mesgNum)),
      profileIndex(Profile::GetFieldIndex(mesgNum, fieldNum)), type(FIT_UINT8_INVALID),
      isFieldExpanded(FIT_FALSE) {}

Field::Field(const std::string &mesgName, const std::string &fieldName)
    : FieldBase(), profile(Profile::GetMesg(mesgName)),
      profileIndex(Profile::GetFieldIndex(mesgName, fieldName)), type(FIT_UINT8_INVALID),
      isFieldExpanded(FIT_FALSE) {}

void Field::SetBaseType(FIT_UINT8 type) { this->type = type; }

FIT_BOOL Field::IsValid(void) const { return profileIndex != FIT_UINT16_INVALID; }

FIT_BOOL Field::GetIsAccumulated() const { return profile->fields[profileIndex].isAccumulated; }
