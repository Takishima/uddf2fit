...
public:
    ...
    void OnMesg(fit::Mesg &mesg) {
        ...
        std::wcout << L"   New Mesg: " << mesg.GetName().c_str() << L".  It has "
                   << mesg.GetNumFields() << L" field(s) and " << mesg.GetNumDevFields()
                   << " developer field(s).\n";

        for (FIT_UINT16 i = 0; i < (FIT_UINT16) mesg.GetNumFields(); i++) {
            fit::Field *field = mesg.GetFieldByIndex(i);
            std::wcout << L"   Field" << i << " (" << field->GetName().c_str() << ") has "
                       << field->GetNumValues() << L" value(s)\n";
            PrintValues(*field);
        }

        for (auto devField : mesg.GetDeveloperFields()) {
            std::wcout << L"   Developer Field(" << devField.GetName().c_str() << ") has "
                       << devField.GetNumValues() << L" value(s)\n";
    ...
    void OnMesg(fit::DiveGasMesg &mesg) override {
        printf("DiveGasMesg:\n");
        if (mesg.IsModeValid())
            printf("   Mode: %d\n", mesg.GetMode());
        ...
        if (mesg.IsHeliumContentValid())
            printf("   Helium: %d\n", mesg.GetHeliumContent());
        if (mesg.IsOxygenContentValid())
            printf("   Oxygen: %u\n", mesg.GetOxygenContent());
    }

    void OnMesg(fit::DiveAlarmMesg &mesg) override {
        printf("DiveAlarm:\n");
        if (mesg.IsIdValid())
            printf("   ID: %d\n", mesg.GetId());
        if (mesg.IsTimeValid())
...
int main(int argc, char *argv[]) {
    fit::Decode decode;
    // decode.SkipHeader();       // Use on streams with no header and footer (stream contains FIT
    // defn and data messages only) decode.IncompleteStream(); // This suppresses exceptions with
    ...
    mesgBroadcaster.AddListener((fit::DiveAlarmMesgListener &) listener);
    mesgBroadcaster.AddListener((fit::DiveGasMesgListener &) listener);
