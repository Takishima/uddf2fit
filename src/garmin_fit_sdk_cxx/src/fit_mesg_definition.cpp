...
void MesgDefinition::SetLocalNum(const FIT_UINT8 newLocalNum) { localNum = newLocalNum; }

void MesgDefinition::AddField(const FieldDefinition &fieldDef) { fields.push_back(fieldDef); }
...
std::vector<DeveloperFieldDefinition> &MesgDefinition::GetDevFields() { return devFields; }
...
const std::vector<FieldDefinition> &MesgDefinition::GetFields() const { return fields; }
...
FIT_BOOL MesgDefinition::Supports(const MesgDefinition &mesgDef) const {
    ...
    for (int i = 0; i < (int) mesgDef.fields.size(); i++) {
        ...
        if (mesgDef.fields[i].GetSize()
            > supportedFieldDef
                  ->GetSize()) // Other field definition is larger than this field definition.
            return FIT_FALSE;
    }
...
const DeveloperFieldDefinition *MesgDefinition::GetDevField(const FIT_UINT8 developerIndex,
...
int MesgDefinition::Write(std::ostream &file) const {
    ...
    if (devFields.size() > 0) {
        file.put((FIT_HDR_TYPE_DEF_BIT | FIT_HDR_DEV_FIELD_BIT)
                 | (localNum & FIT_HDR_TYPE_MASK)); // Message definition record header with local
                                                    // message number.
    } else {
