...
FIT_FLOAT32 FieldBase::GetFLOAT32Value(const FIT_UINT8 fieldArrayIndex,
    ...
    switch (GetType()) { // Note: This checks the type of the MAIN field since data is aligned
...
FIT_WSTRING FieldBase::GetSTRINGValue(const FIT_UINT8 fieldArrayIndex,
    ...
    if (GetType() == FIT_BASE_TYPE_STRING) // Note: This checks the type of the MAIN field since
...
void FieldBase::SetFLOAT64Value(const FIT_FLOAT64 value,
    ...
    FIT_FLOAT64 roundedValue
        = (recalculatedValue >= 0.0) ? (recalculatedValue + 0.5) : (recalculatedValue - 0.5);
...
FIT_BOOL FieldBase::Read(const void *data, const FIT_UINT8 size) {
    ...
    switch (GetType()) {
        ...
        default: {
            ...
            while (bytes > 0) {
                if (memcmp(point, baseTypeInvalids[GetType() & FIT_BASE_TYPE_NUM_MASK], typeSize)
                    != 0) {
