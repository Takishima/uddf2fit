...
#        define FIT_FILE_WEIGHT                                                                   \
            ((FIT_FILE) 9) // Read only, single file. Circular buffer. All message definitions at
                           // start of file. Directory=Weight
...
#        define FIT_MESG_NUM_MFG_RANGE_MIN                                                        \
            ((FIT_MESG_NUM) 0xFF00) // 0xFF00 - 0xFFFE reserved for manufacturer specific messages
#        define FIT_MESG_NUM_MFG_RANGE_MAX                                                        \
            ((FIT_MESG_NUM) 0xFFFE) // 0xFF00 - 0xFFFE reserved for manufacturer specific messages
...
#        define FIT_CHECKSUM_CLEAR                                                                \
            ((FIT_CHECKSUM) 0) // Allows clear of checksum for flash memory where can only write 1
                               // to 0 without erasing sector.
#        define FIT_CHECKSUM_OK                                                                   \
            ((FIT_CHECKSUM) 1) // Set to mark checksum as valid if computes to invalid values 0 or
                               // 0xFF. Checksum can also be set to ok to save encoding computation
                               // time.
...
#        define FIT_DATE_TIME_MIN                                                                 \
            ((FIT_DATE_TIME) 0x10000000) // if date_time is < 0x10000000 then it is system time
...
#        define FIT_LOCAL_DATE_TIME_MIN                                                           \
            ((FIT_LOCAL_DATE_TIME) 0x10000000) // if date_time is < 0x10000000 then it is system
...
typedef FIT_UINT8Z
    FIT_LANGUAGE_BITS_0; // Bit field corresponding to language enum type (1 << language).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_1; // Bit field corresponding to sport enum type (1 << (sport-8)).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_2; // Bit field corresponding to sport enum type (1 << (sport-16)).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_3; // Bit field corresponding to sport enum type (1 << (sport-24)).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_4; // Bit field corresponding to sport enum type (1 << (sport-32)).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_5; // Bit field corresponding to sport enum type (1 << (sport-40)).
...
typedef FIT_UINT8Z
    FIT_SPORT_BITS_6; // Bit field corresponding to sport enum type (1 << (sport-48)).
...
#        define FIT_SUB_SPORT_OBSTACLE                                                            \
            ((FIT_SUB_SPORT) 59) // Used for events where participants run, crawl through mud,
                                 // climb over walls, etc.
...
#        define FIT_SESSION_TRIGGER_AUTO_MULTI_SPORT                                              \
            ((FIT_SESSION_TRIGGER) 2) // Auto multi-sport feature is enabled and user pressed lap
                                      // button to advance session.
#        define FIT_SESSION_TRIGGER_FITNESS_EQUIPMENT                                             \
            ((FIT_SESSION_TRIGGER) 3) // Auto sport change caused by user linking to fitness
                                      // equipment.
...
#        define FIT_EVENT_VIRTUAL_PARTNER_PACE                                                    \
            ((FIT_EVENT) 12) // Group 1. Start at beginning of activity if VP enabled, when VP pace
                             // is changed during activity or VP enabled mid activity. stop_disable
                             // when VP disabled.
...
#        define FIT_EVENT_TIME_DURATION_ALERT                                                     \
            ((FIT_EVENT) 23) // Group 1. Start if enabled mid activity (not required at start of
                             // activity). Stop when duration is reached. stop_disable if
                             // disabled.
#        define FIT_EVENT_DISTANCE_DURATION_ALERT                                                 \
            ((FIT_EVENT) 24) // Group 1. Start if enabled mid activity (not required at start of
                             // activity). Stop when duration is reached. stop_disable if
                             // disabled.
#        define FIT_EVENT_CALORIE_DURATION_ALERT                                                  \
            ((FIT_EVENT) 25) // Group 1. Start if enabled mid activity (not required at start of
                             // activity). Stop when duration is reached. stop_disable if
                             // disabled.
...
#        define FIT_GARMIN_PRODUCT_OHR                                                            \
            ((FIT_GARMIN_PRODUCT) 255) // Garmin Wearable Optical Heart Rate Sensor for ANT+ HR
...
#        define FIT_WORKOUT_CAPABILITIES_TCX                                                      \
            ((FIT_WORKOUT_CAPABILITIES) 0x00000020) // For backwards compatibility. Watch should
                                                    // add missing id fields then clear flag.
#        define FIT_WORKOUT_CAPABILITIES_SPEED                                                    \
            ((FIT_WORKOUT_CAPABILITIES) 0x00000080) // Speed source required for workout step.
...
#        define FIT_WORKOUT_CAPABILITIES_POWER                                                    \
            ((FIT_WORKOUT_CAPABILITIES) 0x00000800) // Power source required for workout step.
#        define FIT_WORKOUT_CAPABILITIES_GRADE                                                    \
            ((FIT_WORKOUT_CAPABILITIES) 0x00001000) // Grade source required for workout step.
...
#        define FIT_COURSE_CAPABILITIES_AVIATION                                                  \
            ((FIT_COURSE_CAPABILITIES) 0x00001000) // Denote course files to be used as flight
                                                   // plans
...
typedef FIT_UINT32
    FIT_WORKOUT_HR; // 0 - 100 indicates% of max hr; >100 indicates bpm (255 max) plus 100
...
typedef FIT_UINT32 FIT_WORKOUT_POWER; // 0 - 1000 indicates % of functional threshold power; >1000
                                      // indicates watts plus 1000.
...
#        define FIT_SWIM_STROKE_IM                                                                \
            ((FIT_SWIM_STROKE) 6) // IM is a mixed interval containing the same number of lengths
                                  // for each of: Butterfly, Backstroke, Breaststroke, Freestyle,
                                  // swam in that order.
...
#        define FIT_LEFT_RIGHT_BALANCE_100_RIGHT                                                  \
            ((FIT_LEFT_RIGHT_BALANCE_100) 0x8000) // data corresponds to right if set, otherwise
                                                  // unknown
...
#        define FIT_CONNECTIVITY_CAPABILITIES_DEVICE_INITIATES_SYNC                               \
            ((FIT_CONNECTIVITY_CAPABILITIES) 0x00008000) // Indicates device is in control of
                                                         // initiating all syncs
...
#        define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_APP_MANAGMENT                            \
            ((FIT_CONNECTIVITY_CAPABILITIES) 0x00100000) // Device supports delete and reorder of
                                                         // apps via GCM
...
#        define FIT_CAMERA_EVENT_TYPE_VIDEO_SPLIT                                                 \
            ((FIT_CAMERA_EVENT_TYPE) 1) // Mark of video file split (end of one file, beginning of
                                        // the other)
...
#        define FIT_EXD_LAYOUT_DYNAMIC                                                            \
            ((FIT_EXD_LAYOUT) 8) // The EXD may display the configured concepts in any layout it
                                 // sees fit.
...
#        define FIT_DIVE_ALARM_TYPE_SPEED                                                         \
            ((FIT_DIVE_ALARM_TYPE) 2) // Alarm when a certain ascent or descent rate is exceeded
...
#        define FIT_PROJECTILE_TYPE_RIFLE_CARTRIDGE                                               \
            ((FIT_PROJECTILE_TYPE) 1) // Rifle cartridge projectile type
#        define FIT_PROJECTILE_TYPE_PISTOL_CARTRIDGE                                              \
            ((FIT_PROJECTILE_TYPE) 2) // Pistol cartridge projectile type
#        define FIT_PROJECTILE_TYPE_SHOTSHELL                                                     \
            ((FIT_PROJECTILE_TYPE) 3) // Shotshell projectile type
#        define FIT_PROJECTILE_TYPE_AIR_RIFLE_PELLET                                              \
            ((FIT_PROJECTILE_TYPE) 4) // Air rifle pellet projectile type
...
#        define FIT_SPLIT_TYPE_TRANSITION                                                         \
            ((FIT_SPLIT_TYPE) 23) // Marks the time going from ascent_split to descent_split/used
                                  // in backcountry ski
