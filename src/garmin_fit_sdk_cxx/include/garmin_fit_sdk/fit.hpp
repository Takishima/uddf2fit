...
#        define FIT_TIMESTAMP_FIELD_NUM                                                           \
            FIT_FIELD_NUM_TIMESTAMP // For reverse compatibility only.  Use FIT_FIELD_NUM_TIMESTAMP
                                    // instead.
...
#        define FIT_STRUCT_OFFSET(MEMBER, STRUCT_TYPE)                                            \
            (((FIT_UINT8 *) &(((STRUCT_TYPE *) FIT_NULL)->MEMBER))                                \
             - ((FIT_UINT8 *) (FIT_NULL))) // Computes the byte offset of a member in a file.
...
#        define FIT_MESG_OFFSET(MESG_MEMBER, MESG_INDEX, MESG_SIZE, FILE)                         \
            (FIT_STRUCT_OFFSET(MESG_MEMBER, FILE) + MESG_INDEX * (FIT_HDR_SIZE + MESG_SIZE)       \
             + FIT_HDR_SIZE) // Computes the byte offset of a message in a file structure. Compiles
                             // to a constant.
...
#        define FIT_MESG_DEF_FIELD_OFFSET(FIELD_MEMBER, FIELD_INDEX)                              \
            (FIT_STRUCT_OFFSET(FIELD_MEMBER, FIT_FIELD_DEF)                                       \
             + FIT_FIELD_DEF_SIZE * FIELD_INDEX) // Computes the byte offset of a field definition
                                                 // member.  Compiles to a constant.
...
typedef std::wstring FIT_WSTRING; // Returned by message/field "Get" APIs and required by "Set"
