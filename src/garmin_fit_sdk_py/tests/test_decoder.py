...
from garmin_fit_sdk import CrcCalculator, Decoder, Stream
...
class TestCheckIntegrity:
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            'Empty File',
            'Invalid Fit File',
            'Minimum Size Fit File',
            'Fit File with Messages',
            'Incorrect Data Size',
        ],
    )
...
class TestDecoderRead:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {'file_id_mesgs': [{'manufacturer': 'garmin', 'type': 'activity', 'time_created': 1000000000}]},
            ),
        ],
    ...
    @pytest.mark.parametrize(
        ...
        [(True, 'activity'), (False, 4), (None, 'activity')],
    ...
    @pytest.mark.parametrize(
        ...
        [(0, pytest.approx(3.0, 0.1)), (2, [-10, 12]), (3, ['Hello!', 'Good Job!'])],
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            'With CRC | Valid File',
            'With CRC | Invalid File',
            'Without CRC | Valid File',
            'Without CRC | Invalid File',
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            'Apply Scale and Offset is True, Little Endian',
            'Apply Scale and Offset is False, Little Endian',
            'Apply Scale and Offset is True, Big Endian',
            'Apply Scale and Offset is False, Big Endian',
        ],
    )
    def test_expand_sub_fields_scale_and_offset(self, option_status, data, distances):
        ...
        duration_distance_workout_step_mesgs = [
            mesg for mesg in messages['workout_step_mesgs'] if 'duration_distance' in mesg
        ]

        for mesg, distance in zip(duration_distance_workout_step_mesgs, distances, strict=False):
...
class TestComponentExpansion:
    ...
    def test_enum_component_expansion(self):
        ...
        assert (
            messages['monitoring_mesgs'][0]['activity_type'] == 'running'
            and messages['monitoring_mesgs'][0]['intensity'] == 3
        )
        assert messages['monitoring_mesgs'][0]['cycles'] == 10

        assert (
            messages['monitoring_mesgs'][1]['activity_type'] == 'walking'
            and messages['monitoring_mesgs'][1]['intensity'] == 0
        )
        assert messages['monitoring_mesgs'][1]['cycles'] == 30

        assert (
            messages['monitoring_mesgs'][2]['activity_type'] == 30 and messages['monitoring_mesgs'][2]['intensity'] == 0
        )
        assert messages['monitoring_mesgs'][2]['cycles'] == 15

        assert (
            'activity_type' not in messages['monitoring_mesgs'][3]
            and 'intensity' not in messages['monitoring_mesgs'][3]
        )
