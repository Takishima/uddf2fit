...
def test_file_header_crc(data, crc_expected, is_correct_crc):
    ...
    if is_correct_crc:
        assert (CrcCalculator.calculate_crc(data, 0, 12) == crc_expected) == is_correct_crc
...
def test_file_crc(data, crc_expected, is_correct_crc, file_length):
    ...
    if is_correct_crc:
        assert (CrcCalculator.calculate_crc(data, 0, file_length) == crc_expected) == is_correct_crc
