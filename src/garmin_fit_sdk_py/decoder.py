...
from . import Accumulator, BitStream, CrcCalculator, fit as FIT, hr_mesg_utils, util
...
class Decoder:
    ...
    def check_integrity(self):
        ...
        try:
            ...
            if file_header.header_size == 14 and file_header.header_crc != CrcCalculator.calculate_crc(
                self._stream.slice(0, 12), 0, 12
            ):
                return False

            file_crc = CrcCalculator.calculate_crc(
                self._stream.read_bytes(file_header.file_total_size), 0, file_header.file_total_size
            )
    ...
    def read(
        ...
        try:
            if self._merge_heart_rates and (not self._apply_scale_and_offset or not self._expand_components):
                self.__raise_error(
                    'merge_heart_rates requires both apply_scale_and_offset and expand_components to be enabled!'
                )
    ...
    def __decode_mesg_def(self):
        ...
        for i in range(mesg_def['num_fields']):
            ...
            num_field_elements = int(
                field_definition['size'] / FIT.BASE_TYPE_DEFINITIONS[field_definition['base_type']]['size']
            )
    ...
    def __read_message(self, mesg_def):
        ...
        for field in mesg_def['field_definitions']:
            ...
            # Fields with an array of values
                ...
                if base_type_definition['type'] == FIT.BASE_TYPE['BYTE']:
                    ...
                    for i in range(num_elements):
                        raw_value = (
                            raw_values[index + i]
                            if raw_values[index + i] != invalid or not convert_invalids_to_none
                            else None
                        )
            ...
            elif raw_values[index] != invalid or not convert_invalids_to_none:
                field_value = raw_values[index]

            if field_value is not None:
                message[field_name] = {'raw_field_value': field_value, 'field_definition_number': field_id}
    ...
    def __expand_components(self, mesg_num, message, fields, mesg_def):
        ...
        while len(self._fields_to_expand) > 0:
            ...
            base_type = (
                FIT.FIELD_TYPE_TO_BASE_TYPE[field_profile['type']]
                if field_profile['type'] in FIT.FIELD_TYPE_TO_BASE_TYPE
                else None
            )
            ...
            for i in range(len(field_profile['components'])):
                ...
                if target_field['name'] not in mesg:
                    base_type = (
                        FIT.FIELD_TYPE_TO_BASE_TYPE[target_field['type']]
                        if target_field['type'] in FIT.FIELD_TYPE_TO_BASE_TYPE
                        else target_field['type']
                    )
                    invalid_value = (
                        FIT.BASE_TYPE_DEFINITIONS[base_type]['invalid']
                        if base_type in FIT.BASE_TYPE_DEFINITIONS
                        else 0xFF
                    )
    ...
    def __expand_sub_fields(self, global_mesg_num, message):
        ...
        for field in self._fields_with_subfields:
            if message[field]['field_definition_number'] in Profile['messages'][global_mesg_num]['fields']:
                field_profile = Profile['messages'][global_mesg_num]['fields'][
                    message[field]['field_definition_number']
                ]
            else:
                continue
        ...
        try:
            ...
            if isinstance(raw_field_value, list):
                for i in range(len(raw_field_value)):
                    field_value[i] = (
                        types[str(raw_field_value[i])] if str(raw_field_value[i]) in types else raw_field_value[i]
                    )
                return field_value
    ...
    def __apply_scale_and_offset(self, field_profile, raw_field_value):
        ...
        try:
            ...
            if isinstance(raw_field_value, list):
                for i in range(len(raw_field_value)):
                    field_value = (
                        raw_field_value[i] / scale
                        if (raw_field_value[i] is not None and scale != 1)
                        else raw_field_value[i]
                    )
    ...
    def __add_developer_data_id_to_profile(self, message):
        if (
            message is None
            or message['developer_data_index'] is None
            or message['developer_data_index']['raw_field_value'] == 0xFF
        ):
            return
        ...
        self._developer_data_defs[message['developer_data_index']['raw_field_value']] = {
            ...
            'application_version': message['application_version']['raw_field_value']
            if 'application_version' in message
            else None,
            'fields': [],
        }
    ...
    def __add_field_description_to_profile(self, message):
        ...
        self._developer_data_defs[message['developer_data_index']['raw_field_value']]['fields'].append({
            ...
            'fit_base_type_id': message['fit_base_type_id']['raw_field_value']
            if 'fit_base_type_id' in message
            else None,
            ...
            'fit_base_unit_id': message['fit_base_unit_id']['raw_field_value']
            if 'fit_base_unit_id' in message
            else None,
            'native_mesg_num': message['native_mesg_num']['raw_field_value'] if 'native_mesg_num' in message else None,
            'native_field_num': message['native_field_num']['raw_field_value']
            if 'native_field_num' in message
            else None,
            'key': message['key'],
        })
            ...
            for field in message:
                if isinstance(message[field], dict) and 'raw_field_value' in message[field]:
                    message[field] = (
                        ...
                        if 'field_value' in message[field]
                        else message[field]['raw_field_value']
                    )
