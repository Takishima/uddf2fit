        ...
        while finding_in_range_hr_mesgs and (heartrate_index < len(heartrates)):
            ...
            # Check if the heartrate timestamp exceeds the record time
                ...
                if hr_sum_count > 0:
                    ...
                    avg_hr = int((hr_sum / hr_sum_count) + 0.5)
...
def expand_heart_rates(hr_mesgs):
    ...
    for message in hr_mesgs:
        ...
        event_timestamps = (
            message['event_timestamp'] if isinstance(message['event_timestamp'], list) else [message['event_timestamp']]
        )
        filtered_bpm = (
            message['filtered_bpm'] if isinstance(message['filtered_bpm'], list) else [message['filtered_bpm']]
        )
        ...
        for i in range(len(event_timestamps)):
            ...
            if event_timestamp < anchor_event_timestamp:
                if anchor_event_timestamp - event_timestamp > (0x400000):
                    ...
                    __raise_error(
                        'Anchor event_timestamp is greater than subsequent event_timestamp. This does not allow for correct delta calculation.'
                    )
